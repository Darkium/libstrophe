			    ...
			    void * const userdata)
{
	...
	for (mech = xmpp_stanza_get_children(child); mech;
		...
		else if (strcasecmp(text, "X-FACEBOOK-PLATFORM") == 0)
		    conn->sasl_support |= SASL_MASK_FACEBOOK;
...
static int _handle_facebook_challenge(xmpp_conn_t * const conn,
    ...
    name = xmpp_stanza_get_name(stanza);
    xmpp_debug(conn->ctx, "xmpp",\
        "handle facebook digest (challenge) called for %s", name);
...
static void _auth(xmpp_conn_t * const conn)
{
	...
	xmpp_disconnect(conn);
    } else if (conn->sasl_support & SASL_MASK_FACEBOOK) {
	auth = _make_sasl_auth(conn, "X-FACEBOOK-PLATFORM");
	if (!auth) {
		disconnect_mem_error(conn);
		return;
	}
